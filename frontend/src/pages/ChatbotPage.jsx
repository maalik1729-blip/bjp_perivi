...
// ── Long text step (work / experience, local area, strategy) ─
function LongTextMsg({ active, title, icon, prompt, initial, onSubmit, disabled, sampleChips, isOptional }) {
  const { t } = useLang()
      ...
      {sampleChips && (
        <div className="sample-chip-container">
          {sampleChips.map((chip, idx) => (
            <span key={idx} className="sample-chip" onClick={() => active && setText((prev) => (prev ? `${prev}\n${chip}` : chip))}>
              + {chip}
            </span>
          ))}
        </div>
      )}
      <textarea value={text} disabled={!active} onChange={(e) => setText(e.target.value)} rows={5}
...
// ── Photo upload component (Step 1) ──────────────────────
function PhotoUploadMsg({ active, initialUrl, onSubmit, disabled }) {
  const { t } = useLang()
    ...
    <div className="interactive-card">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
        {photoUrl ? (
          ...
          <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'var(--color-abyss)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px dashed var(--color-graphite)' }}>
...
// ── Organiser notice box (Step 9) ────────────────────────
function OrganiserNoticeMsg() {
  const { t } = useLang()
  return (
    <div className="organiser-notice-box">
      <i className="bi bi-shield-lock-fill" style={{ color: '#ff9933', marginRight: 6 }} />
      {t('⚠️ Application Submitted & Verified. If you need to make changes to your application, please get in touch directly with the campaign organiser.')}
    </div>
  )
}
...
function CandidateCardMsg({ result, appData }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = image
      link.download = `CandidateCard_${result?.application_id || 'BJP'}.png`
      link.click()
    } catch (e) {
...
function ReviewMsg({ active, data, mobile, onConfirm, onEdit, disabled }) {
  ...
  return (
    <div style={cardBox}>
      ...
      {/* Ward strategy — editable */}
      {cur.wardStrategy && (
        <ReviewSection title={t('What strategy do you have to win this ward?')} icon="bullseye">
          {!editing ? (
            <div style={{ fontSize: 13, color: 'var(--color-chalk)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{cur.wardStrategy}</div>
          ) : (
            <textarea style={{ ...controlStyle, resize: 'vertical', fontFamily: 'inherit' }} rows={4}
              value={draft.wardStrategy} onChange={(e) => setDraftField({ wardStrategy: e.target.value })} />
          )}
        </ReviewSection>
      )}
      ...
      {/* Campaign video */}
      {cur.videoUrl && (
        <ReviewSection title={t('Campaign / Intro Video')} icon="camera-reels-fill">
          <KV k={t('Video Link')} v={cur.videoUrl} />
        </ReviewSection>
      )}

      {/* Extra details */}
      {(cur.extraQ1 || cur.optionalProfile) && (
        <ReviewSection title={t('Additional Details & Profile')} icon="patch-question-fill">
          <KV k={t('Previous Experience')} v={cur.extraQ1} />
          <KV k={t('Profile / Bio Link')} v={cur.optionalProfile} />
        </ReviewSection>
      )}
...
// ── Post-submission optional profile component ─────────────
function PostSubmitProfileMsg({ initialProfile, onSave, onSkip }) {
  const { t } = useLang()
  ...
  if (saved) {
    return (
      <div style={{ fontSize: 13, color: 'var(--color-signal-mint)', fontWeight: 600, padding: 12, textAlign: 'center', background: 'rgba(46,204,113,0.08)', borderRadius: 10, border: '1px solid rgba(46,204,113,0.2)', width: '100%' }}>
        <i className="bi bi-check-circle-fill" /> {profileUrl ? t('Profile link saved ✓') : t('Skipped optional profile')}
      </div>
    )
  }
  ...
  return (
    <div style={{ ...cardBox, marginTop: 10 }}>
      <div style={cardTitle}><i className="bi bi-link-45deg" /> {t('Profile / Bio Link (Optional)')}</div>
      <div style={{ fontSize: 12, color: 'var(--color-ash)', textAlign: 'left', lineHeight: 1.4 }}>
        {t('Add an optional link to your resume, website, or professional profile.')}
      </div>
      <input
        style={controlStyle}
        type="url"
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
        <button
          type="button"
          style={{ ...primaryBtn(true), background: 'var(--color-graphite)', flex: 1 }}
          onClick={() => { setSaved(true); if (onSkip) onSkip() }}
        >
          {t('Skip')}
        </button>
        <button
          type="button"
          style={{ ...primaryBtn(true), flex: 1 }}
          onClick={() => { setSaved(true); if (onSave) onSave(profileUrl.trim()) }}
        >
          <i className="bi bi-check-lg" /> {t('Save Profile')}
        </button>
      </div>
    </div>
  )
...
function SubmittedMsg({ result, alreadyApplied, appData, onSaveProfile }) {
  const { t } = useLang()
  return (
    <div style={{ ...cardBox, alignItems: 'center', textAlign: 'center' }}>
      ...
      <CandidateCardMsg result={result} appData={appData} />

      <PostSubmitProfileMsg
        initialProfile={appData?.optionalProfile}
        onSave={(url) => onSaveProfile && onSaveProfile(url)}
        onSkip={() => onSaveProfile && onSaveProfile('')}
      />
...
export default function ChatbotPage() {
  ...
  const handleConfirmVoter = async () => {
    ...
    await botSay(t('Please upload your passport photo (or enter photo URL).'), 350)
    addMsg('bot', 'photo', {})
  ...
  const handlePhotoSubmit = async (photoUrl) => {
    patchData({ photoUrl })
    addMsg('user', 'text', { text: t('Candidate photo updated ✓') })
  ...
  const handleWorkSubmit = async (text) => {
    ...
    await botSay(t('What strategy do you have to win this ward? Describe your ground campaign plan (max 500 words).'), 350)
    addMsg('bot', 'ward_strategy', {})
    setChatState(S.WARD_STRATEGY)
  }

  const handleWardStrategySubmit = async (text) => {
    patchData({ wardStrategy: text })
    addMsg('user', 'text', { text: t('Ward strategy added ✓') })
    await botSay(t('Campaign / Intro Video — enter video link or URL (optional).'), 350)
    addMsg('bot', 'video', {})
    setChatState(S.AWAIT_VIDEO)
  }

  const handleVideoSubmit = async (text) => {
    patchData({ videoUrl: text })
    addMsg('user', 'text', { text: t('Campaign video added ✓') })
    await botSay(t('Previous political / election experience & campaign commitment (optional).'), 350)
    addMsg('bot', 'extra_questions', {})
    setChatState(S.EXTRA_QUESTIONS)
  }

  const handleExtraQuestionsSubmit = async ({ extraQ1, extraQ2 }) => {
    patchData({ extraQ1, extraQ2 })
    addMsg('user', 'text', { text: t('Additional details saved ✓') })
  ...
  const handleProfileSavePostSubmit = (url) => {
    patchData({ optionalProfile: url })
  }
  ...
  const handleReviewConfirm = async () => {
    ...
    try {
      const payload = {
        ...
        ward_strategy: d.wardStrategy,
        video_url: d.videoUrl,
        extra_question_1: d.extraQ1,
        extra_question_2: d.extraQ2,
        optional_profile: d.optionalProfile,
  ...
  const renderMsgContent = (msg) => {
    ...
    switch (msg.type) {
      ...
      case 'photo':
        return (
          <PhotoUploadMsg
            active={isLatest && chatState === S.AWAIT_PHOTO}
            initialUrl={appData.photoUrl}
      ...
      case 'work':
        return (
          <LongTextMsg
            ...
            icon="briefcase-fill"
            prompt="Describe your work, party service, and community experience (max 500 words)."
            sampleChips={[
              'Active BJP party worker for 5 years, organized 10+ welfare camps.',
              'Participated in local ward development, youth outreach, and party events.',
              'Managed election booth operations and community grievance resolution.',
            ]}
      ...
      case 'ward_strategy':
        return (
          <LongTextMsg
            active={isLatest && chatState === S.WARD_STRATEGY}
            title="What strategy do you have to win this ward?"
            icon="bullseye"
            prompt="Describe your ground campaign plan, voter outreach, and strategy to win your ward (max 500 words)."
            initial={appData.wardStrategy}
            onSubmit={handleWardStrategySubmit}
            disabled={isTyping}
          />
        )
      case 'video':
        return (
          <LongTextMsg
            active={isLatest && chatState === S.AWAIT_VIDEO}
            title="Campaign / Intro Video"
            icon="camera-reels-fill"
            prompt="Paste a video URL or link (e.g., YouTube / Drive / MP4) introducing yourself."
            initial={appData.videoUrl}
      ...
      case 'extra_questions':
        return (
          <LongTextMsg
            active={isLatest && chatState === S.EXTRA_QUESTIONS}
            title="Experience & Commitment"
            icon="patch-question-fill"
            prompt="Share your previous political/election experience and daily time commitment."
            initial={appData.extraQ1}
            onSubmit={(text) => handleExtraQuestionsSubmit({ extraQ1: text, extraQ2: 'Full Time' })}
            disabled={isTyping}
            isOptional={true}
          />
        )
      ...
      case 'local_area':
        return (
          <LongTextMsg
            active={isLatest && (chatState === S.LOCAL_AREA || chatState === 'PROFILE_OPTIONAL')}
      ...
      case 'submitted':
        return <SubmittedMsg result={msg.result} alreadyApplied={msg.alreadyApplied} appData={appData} onSaveProfile={handleProfileSavePostSubmit} />
      default:
    ...
    <div className="chatbot-app bjp-theme">
      <div className="chatbot-fullpage">
        <div className="chatbot-container">
          ...
          {/* Membership Banner (Step 6) */}
          {chatState === S.AWAIT_MEMBERSHIP && (
            <div className="membership-banner" style={{ margin: '0 16px 8px 16px' }}>
              <span style={{ fontSize: 12, color: 'var(--color-ash)' }}>
                <i className="bi bi-info-circle-fill" style={{ color: '#ff9933', marginRight: 4 }} />
                {t('Not a BJP Member yet?')}
              </span>
              <a href="https://www.bjp.org" target="_blank" rel="noopener noreferrer" className="membership-link-btn">
                {t("Don't have a BJP Membership ID? Click here to register on BJP Portal →")}
              </a>
            </div>
          )}
