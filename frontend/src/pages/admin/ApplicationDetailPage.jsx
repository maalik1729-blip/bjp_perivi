    ...
    <div className="admin-detail-view">
      ...
      <div className="admin-card candidate-hero-card">
        <div className="candidate-hero-body">
          <div className="candidate-avatar-large">
            {app.photo_url ? (
              <img src={app.photo_url} alt={v.name || 'Candidate'} style={{ objectFit: 'cover', borderRadius: '50%' }} />
            ) : (
      ...
      <div className="detail-cards-grid">
        ...
        <DetailSection title="Personal & Voter Details" icon="person-badge-fill">
          ...
          {app.optional_profile && <DetailRow icon="link-45deg" label="Optional Profile / Bio" value={app.optional_profile} />}
        </DetailSection>
        ...
        {/* Ward Strategy */}
        {app.ward_strategy && (
          <DetailSection title="Ward Victory Strategy" icon="bullseye">
            <div className="text-content-box">{app.ward_strategy}</div>
          </DetailSection>
        )}

        {/* Campaign Video */}
        {app.video_url && (
          <DetailSection title="Campaign / Intro Video" icon="camera-reels-fill">
            <div className="text-content-box">
              <a href={app.video_url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-saffron)', fontWeight: 600 }}>
                <i className="bi bi-play-circle-fill me-2" /> View Candidate Video ({app.video_url})
              </a>
            </div>
          </DetailSection>
        )}
        ...
        {/* Extra Questions */}
        {(app.extra_question_1 || app.extra_question_2) && (
          <DetailSection title="Additional Questions" icon="patch-question-fill">
            {app.extra_question_1 && <DetailRow icon="journal-check" label="Previous Experience" value={app.extra_question_1} />}
            {app.extra_question_2 && <DetailRow icon="clock-history" label="Availability & Commitment" value={app.extra_question_2} />}
          </DetailSection>
        )}
        ...
        {/* Organiser Notice */}
        <DetailSection title="Post-Submission Policy & Lock Status" icon="shield-lock-fill">
          <div className="text-content-box" style={{ borderLeft: '4px solid var(--color-saffron)' }}>
            ⚠️ <strong>Application Locked:</strong> Record submitted by candidate. Any candidate profile updates must be coordinated through campaign leadership/organisers.
          </div>
        </DetailSection>
      </div>
    </div>
