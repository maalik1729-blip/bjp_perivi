...
export async function postSubmitApplication(req, res) {
  ...
  // New candidate fields (from 9-step requirements)
  const photoUrl = String(body.photo_url || '').trim()
  const videoUrl = String(body.video_url || '').trim()
  const wardStrategy = String(body.ward_strategy || '').trim()
  const extraQuestion1 = String(body.extra_question_1 || '').trim()
  const extraQuestion2 = String(body.extra_question_2 || '').trim()
  const optionalProfile = String(body.optional_profile || '').trim()
  ...
  try {
    const { application_id, submitted_at } = await createApplication({
      ...
      extra_question_1: extraQuestion1,
      extra_question_2: extraQuestion2,
