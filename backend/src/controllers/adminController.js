...
import { listApplications, getStats, getTopAssemblies, getReport, findApplicationById, deleteApplicationById } from '../models/applicationModel.js'
...
export async function deleteApplication(req, res) {
  if (!isAppDbOnline()) return res.status(503).json({ success: false, message: 'Application database unavailable.' })
  const success = await deleteApplicationById(req.params.id)
  if (success) {
    return res.json({ success: true, message: `Application ${req.params.id} deleted.` })
  }
