...
async function assemblyCollections(db) {
  const now = Date.now()
  if (_assemblyCollCache && _assemblyCollCache.length > 0 && now - _assemblyCollCacheAt < COLL_CACHE_TTL) {
...
export async function findVoterByEpic(epicNo) {
  ...
  let names = await assemblyCollections(db)
  if (!names.length) {
    try {
      const { getAppClient } = await import('../config/db.js')
      const client = getAppClient()
      if (client) {
        const voterAlt = client.db('voter_db')
        const altNames = await assemblyCollections(voterAlt)
        if (altNames.length) {
          db = voterAlt
          names = altNames
        }
